...
export function ChatPanel({ notebook }: Props) {
  ...
  return (
    <section className="flex h-full min-h-0 flex-col bg-transparent">
      {/* Header */}
      <header className="flex items-center justify-between px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-[#0a0a0a]/80 backdrop-blur-sm">
        <div className="flex items-center gap-3 text-sm font-semibold tracking-tight text-zinc-950 dark:text-zinc-50">
          ...
          Session
          {notebook && (
            <span className="text-zinc-500 font-medium hidden sm:inline px-2 py-0.5 rounded-md bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-[10px] uppercase tracking-widest ml-2">
              Context: {notebook.name}
            </span>
          )}
        </div>
        <div className="flex items-center gap-2 px-2.5 py-1 rounded-md bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-[10px] font-semibold uppercase tracking-wider text-zinc-600 dark:text-zinc-400">
      ...
      <div className="p-6 bg-white dark:bg-[#0a0a0a]">
        <form onSubmit={onSubmit} className="mx-auto max-w-3xl relative">
          <div
            className={cn(
              "group relative flex flex-col rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#0a0a0a] p-2 shadow-sm transition-all duration-200 focus-within:border-zinc-300 dark:focus-within:border-zinc-700",
          ...
          >
            <textarea
              ...
              rows={2}
              className="w-full bg-transparent p-3 text-sm text-zinc-950 dark:text-zinc-50 placeholder:text-zinc-400 outline-none resize-none min-h-[60px] max-h-[200px]"
            />
               ...
               <div className="flex items-center gap-2">
                  {isStreaming ? (
                    ...
                    <Button
                      ...
                      size="sm"
                      className="h-8 rounded-md bg-zinc-950 dark:bg-zinc-50 text-white dark:text-zinc-950 hover:bg-zinc-800 dark:hover:bg-zinc-200 transition-colors shadow-none"
                    >
                      Send
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center text-center pb-20 overflow-hidden">
      <div className="relative mb-8">
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
          <MessageSquare className="h-8 w-8 text-zinc-400" />
        </div>
      </div>
      
      <h3 className="mb-2 text-2xl font-semibold tracking-tight text-zinc-950 dark:text-zinc-50">
        Ready for queries.
      </h3>
      <p className="mb-8 max-w-sm text-sm text-zinc-500 leading-relaxed">
        Synchronize your research documents to begin dialogue.
      </p>
      ...
      <div className="grid w-full max-w-lg grid-cols-1 gap-2 sm:grid-cols-2">
        {SUGGESTED_PROMPTS.map((p) => (
          <button
            ...
            disabled={disabled}
            onClick={() => onPick(p)}
            className="group flex items-center justify-between rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 p-4 text-left text-xs font-medium text-zinc-600 dark:text-zinc-400 transition-colors hover:bg-zinc-100 dark:hover:bg-zinc-900 hover:text-zinc-950 dark:hover:text-zinc-50 disabled:cursor-not-allowed disabled:opacity-50"
          >
