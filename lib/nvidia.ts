...
export async function embed(text: string, inputType: InputType): Promise<number[]> {
  const key = getApiKey()
  if (!key) {
    console.warn("NVIDIA_API_KEY missing, using mock embedding")
...
export async function embedBatch(texts: string[], inputType: InputType): Promise<number[][]> {
  const key = getApiKey()
  if (!key) {
    console.warn("NVIDIA_API_KEY missing, using mock batch embeddings")
...
export async function* streamChat(messages: ChatMessage[]): AsyncGenerator<string> {
  ...
  if (!key) {
    ...
    const dummyWords = "This is a mock response because no NVIDIA API key was provided. To see real intelligence, configure your .env file with an NVIDIA_API_KEY and restart the app.".split(" ")
